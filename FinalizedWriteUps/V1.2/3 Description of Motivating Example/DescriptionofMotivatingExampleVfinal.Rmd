...
The QC measures employed by Arazi, Rao, Berthier, et al. in [@arazi2018immune] are:

1. $1,000 < \mathbf{nFeature} < 5,000$
2. $\mathbf{PerctMT} \leq 25 \%$

All observations for which the calculated values of $\mathbf{nFeature}$ and $\mathbf{PerctMT}$ satisfied the inequalites in (1) and (2) above were kept, and the others were considered "low-quality" and removed.  The resulting distribution of the $\mathbf{PerctMT}$ variable is displayed in (\textbf{Figure 2}):
...
As 84% of cells as removed with the filters chosen by Arazi et al, I choose a more lenient threshold, removing observations with $\mathbf{PerctMT} \leq 60 \%$ to keep more cells.

The additional subsetting measure of restricting the data to only B-cells is also made in an effort to regularize the data sample (i.e. homogenize feature expression).  The resulting distribution of $\mathbf{PerctMT}$ is displayed in (\textbf{Figure 3}) after filtering.
...
The distribution of observations for each subject before and after the quality control thresholds are imposed is also show numerically in \textbf{Table 1}:
...
\hline
\hline
\textbf{Number of Observations Before QC} & 327 & 311 & 379 & 375 & 345 & 371 & 381 & 381 & 377 \\
\hline
\textbf{Number of Observations After QC} & 21 & 107 & 0 & 107 & 100 & 25 & 0 & 122 & 0 \\
\hline
\end{tabular}
...
\hline
\hline
Number of Observations Before QC & 380 & 381 & 380 & 333 & 333 & 239 & 218 & 378 & 342 \\
\hline
Number of Observations After QC & 127 & 75 & 0 & 87 & 0 & 79 & 0 & 53 & 0 \\
\hline
\end{tabular}
\end{flushleft}
